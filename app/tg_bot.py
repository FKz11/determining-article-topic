...
TOKEN_TELEGRAM_API = (
    s3_client.get_object(Bucket=BUCKET, Key="secrets/tg_bot/token.txt")
    .get("Body")
    .read()
    .decode()
)
...
async def predict_file(update, context):
    ...
    async with httpx.AsyncClient() as client:
        for try_num in range(1, 4):
            ...
            num_hubs = context.chat_data.get("num_hubs", NUM_HUBS)
            response = await client.post(
                service_host_url + "/predict_file" + f"?num_hubs={num_hubs}",
                files={
                    "file": ("filename", io.BytesIO(file_response.content))
                },
            )
            if response.status_code != 200:
                message = f"Ошибка: {response.status_code}, попыток: {try_num}"
...
async def button_click(update, context):
    ...
    async with httpx.AsyncClient() as client:
        for try_num in range(1, 4):
            response = await client.post(
                service_host_url + "/feedback" + f"?button={button}"
            )
            if response.status_code != 200:
                await asyncio.sleep(2**try_num)
                continue
...
async def rating(update, context):
    async with httpx.AsyncClient() as client:
        for try_num in range(1, 4):
            ...
            if num_feedbacks == 0:
                message = (
                    "Пока нету ни одного отзыва о рекомендациях, стань первым!"
                )
            else:
